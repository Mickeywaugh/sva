...
function loadFromStorage(): RecentMenuItem[] {
  try {
    const data = localStorage.getItem(STORAGE_KEYS.USER_RECENT_MENUS);
...
function saveToStorage(menus: RecentMenuItem[]) {
  localStorage.setItem(STORAGE_KEYS.USER_RECENT_MENUS, JSON.stringify(menus));
}
...
export function useRecentMenus() {
  ...
  function clearRecentMenus() {
    recentMenus.value = [];
    localStorage.removeItem(STORAGE_KEYS.USER_RECENT_MENUS);
  }
