      ...
      <el-form ref="queryFormRef" :model="queryParams" v-bind="formAttrs" :class="isGrid">
        <template v-for="(item, index) in formItems" :key="item.prop">
          <el-form-item v-show="isExpand ? true : index < showNumber" :label="item?.label" :prop="item.prop">
            ...
            <!-- 自定义插槽 -->
            <slot v-if="item.type === 'custom'" :name="item.slotName" :form-data="queryParams" :prop="item.prop"
              :attrs="{ style: { width: '100%' }, ...item.attrs }" />
            <el-cascader v-else-if="item.type === 'cascader'" v-model.trim="queryParams[item.prop]"
