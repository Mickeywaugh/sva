<template>
        ...
        <template v-if="list.length > 0">
          <div v-for="item in list" :key="item.id" class="w-500px py-3">
            <div class="flex-y-center">
              ...
              <el-text size="small" class="w-200px cursor-pointer !ml-2 !flex-1" truncated @click="read(item.id)">
                {{ item.title }}
              </el-text>
  ...
  <el-dialog v-model="dialogVisible" :title="detail?.title ?? '通知详情'" width="800px" custom-class="notification-detail">
