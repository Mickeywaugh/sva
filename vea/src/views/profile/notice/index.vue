<template>
  <div class="app-container">
    ...
    <el-card class="table-card" shadow="never">
        ...
        <div class="left-toolbar">
          <el-form ref="queryFormRef" :model="pageData.params" :inline="true">
            <el-form-item label="通知标题" prop="title">
              <el-input v-model="pageData.params.title" placeholder="关键字" clearable @keyup.enter="handleQuery" />
            </el-form-item>
            <el-form-item>
      ...
      <el-table ref="dataTableRef" v-loading="loading" :data="pageData.list" highlight-current-row>
      ...
      <pagination v-if="total > 0" v-model:total="total" v-model:page="pageData.params.pageNum" v-model:limit="pageData.params.pageSize"
        @pagination="handleQuery" />
    </el-card>

    <el-dialog v-model="noticeDialogVisible" :title="noticeDetail?.title ?? '通知详情'" width="800px" custom-class="notice-detail">
...
<script setup lang="ts">
  ...
  import type { NoticeDetail, NoticeItem } from "@/api/system/notice";

  const queryFormRef = ref();
  const loading = ref(false);
  const total = ref(0);

  const pageData = reactive<PageResult<NoticeItem>>({
    list: [],
    total: 0,
    params: {
      pageNum: 1,
  ...
  async function handleQuery() {
    loading.value = true;
    try {
      const data = await NoticeAPI.getMyNoticePage(pageData.params);
  ...
  async function handleReadNotice(id: number) {
