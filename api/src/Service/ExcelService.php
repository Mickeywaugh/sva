...
class ExcelService
{
  ...
  public function genDeliveryNote(array $data, string $fileName): bool
  {
    ...
    try {
      ...
      if (false ==  BaseService::mkdirP($fileName)) return false;
      $styleAll = array(
        'alignment' => [
          'horizontal' => "center",
          'vertical' => "center"
      ...
      $sheet->getStyle('A1')->applyFromArray($styleAll);
      ...
      $sheet->getStyle('A1:G' . $row)->applyFromArray($styleAll);
      ...
      BaseService::log("生成发货单成功：" . $fileName);
      return true;
    } catch (\Exception $e) {
      BaseService::log($e->getMessage(), "error");
      return false;
    }
  }
}
